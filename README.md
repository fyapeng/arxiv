...
<!-- ARXIV_PAPERS_START -->
*(Updated on: 2025-06-30 ET)*

1. **[Independence Axioms in Social Ranking](https://arxiv.org/abs/arXiv:2506.21836)**<br/>社会排名中的独立性公理
    - *Authors: Takahiro Suzuki, Michele Aleandri, Stefano Moretti*
2. **[On the hidden costs of passive investing](https://arxiv.org/abs/arXiv:2506.21775)**<br/>被动投资的隐性成本
    - *Authors: Iro Tasitsiomi*
3. **[Optimal Estimation of Two-Way Effects under Limited Mobility](https://arxiv.org/abs/arXiv:2506.21987)**<br/>在有限流动性下对双向效应的最优估计
    - *Authors: Xu Cheng, Sheng Chao Ho, Frank Schorfheide*
4. **[Optimal Benchmark Design under Costly Manipulation](https://arxiv.org/abs/arXiv:2506.22142)**<br/>在成本高昂的操作下的最佳基准设计
    - *Authors: Ángel Hernando-Veciana*
5. **[Monetary Macro Accounting Theory](https://arxiv.org/abs/arXiv:2506.21651)**<br/>货币宏观核算理论
    - *Authors: Renéee Menéndez, Viktor Winschel*
6. **[Multi-to one-dimensional screening and semi-discrete optimal transport](https://arxiv.org/abs/arXiv:2506.21740)**<br/>多维到一维筛选与半离散最优传输
    - *Authors: Omar Abdul Halim, Brendan Pass*
...
### 社会排名中的独立性公理
**[Independence Axioms in Social Ranking](https://arxiv.org/abs/arXiv:2506.21836)**
...
**摘要**: 在社会选择理论中，独立于非基本输入信息的变化是一个被广泛认可的公理。这种独立性降低了指定和/或分析非基本数据的成本。本研究对社会排名解决方案（SRSs）背景下的独立公理进行了全面分析。我们考虑了七个独立公理（其中两个是新的），并通过在现有交集初始段规则的特征描述中替换这些独立公理，提供了字典序卓越解决方案和多数决的新特征描述。这些特征描述突出了三种SRSs在独立性方面的差异。

---
### 被动投资的隐性成本
**[On the hidden costs of passive investing](https://arxiv.org/abs/arXiv:2506.21775)**
...
**摘要**: 被动投资因其低费用和看似简单的专注于零跟踪误差而非证券选择而获得了巨大的流行。然而，我们的分析显示，被动（零跟踪误差）的方法——在指数重组当天市场收盘时购买股票（该股票几天前已宣布将被加入指数）——相比于提前逐步收购一小部分所需股份且跟踪误差最小的策略，会导致成本高达数百个基点。此外，我们展示了在所有分析的情况下，一个在公告后建立小库存并在重组事件中提供流动性的交易者可以持续获得数百个基点的利润，通常更多，假设风险最小。

---
### 在有限流动性下对双向效应的最优估计
**[Optimal Estimation of Two-Way Effects under Limited Mobility](https://arxiv.org/abs/arXiv:2506.21987)**
...
**摘要**: 我们提出了一种基于新颖先验的实证贝叶斯估计器，用于分析链接数据集中的双向效应，该先验利用了数据中观察到的正配对匹配模式。为了捕捉有限的流动性，我们在渐近框架中对匹配数据相关的二分图进行建模，其中其拉普拉斯矩阵具有收敛至零的小特征值。控制收缩的先验超参数通过最小化无偏风险估计来确定。我们展示了所提出的实证贝叶斯估计器在复合损失中是渐近最优的，尽管二分图的连通性较弱，且先验可能存在误设定。我们从链接的北卡罗来纳州教育研究数据中心学生-教师数据集中估计了教师的附加值。

---
### 在成本高昂的操作下的最佳基准设计
**[Optimal Benchmark Design under Costly Manipulation](https://arxiv.org/abs/arXiv:2506.22142)**
...
**摘要**: 价格基准被用来将市场价格趋势纳入合同中，但其使用可能为合同各方创造操纵的机会。本文使用一个受以太坊等区块链上的智能合约启发的现实且可行的模型来检验这一问题。在我们的模型中，操纵成本取决于两个因素：对个别价格调整的幅度（可变成本）和调整的价格数量（固定成本）。我们发现，当固定成本可以忽略不计时，加权平均是最优基准，而当可变成本可以忽略不计时，中位数是最优基准。在固定成本和可变成本都显著的情况下，最优基准可以作为一个修剪均值来实施，随着固定成本相对于可变成本变得更为重要，修剪的程度也会增加。此外，我们展示了基于均值的基准的最优权重与边际操纵成本成比例，而即使在不同价格的固定成本不同的情况下，中位数仍然是无需加权的最优选择。

---
### 货币宏观核算理论
**[Monetary Macro Accounting Theory](https://arxiv.org/abs/arXiv:2506.21651)**
...
**摘要**: 我们发展了一种货币宏观会计理论（MoMaT）及其软件规范，以实现一致的国家会计。在我们的货币理论中，货币主要作为支付义务和债务的媒介，而不是交换媒介，这源于生产者在收到收入之前先向供应商支付的时间错位。MoMaT应用了分离和抽象的法律原则来模拟债务、合同、财产权和货币，以理解它们的本质。根据我们的方法，货币系统在三个相互连接的层面上运作：微观（劳动分工）、中观（银行风险共担）和宏观（GDP共享、货币发行）。货币理论的关键是对宏观债务关系的理解，因此模型关注的不是货币的流通，而是债务漩涡：金融义务的持续创造和解决。汇票（BoE）作为一种统一的合同工具，连接了债务流程和货币发行，跨越了法定货币和基于黄金的系统。多层次的BoE框架支持流动性交换、投资和背书，旨在潜在地在区块链智能合约和AI自动化中实现，以提高借贷透明度。通过范畴论和层论来确保数学严谨性，用于经济层面之间的不变量，同调论用于货币政策基础。开放游戏可以利用多智能体模型构建宏观经济分析，使MoMaT适用于区块链经济理论、货币政策和供应链金融。

---
### 多维到一维筛选与半离散最优传输
**[Multi-to one-dimensional screening and semi-discrete optimal transport](https://arxiv.org/abs/arXiv:2506.21740)**
...
**摘要**: 我们研究了在消费者具有多维分布和商品空间为一维的情况下，垄断者的筛选问题。我们建立了一般条件，在此条件下，解决方案满足一个被称为嵌套性的结构条件，这大大简化了它们的分析和特征描述。在这些假设下，我们继续开发一种通用方法来解决问题，无论是通过封闭形式还是相对简单的数值计算，并用例子来说明。这些结果既在垄断者只能访问一维产品空间的离散子集时成立，也在整个连续统可用时成立。在前一种情况下，我们还建立了一个唯一性结果。
